...
public class MemoryFileCache {
	...
	public MemoryFileItem get (String name) throws IOException{
			...
			if (o instanceof byte[]){
				retval = new MemoryFileItem (name,"text/plain",false,name, 0);
				retval.getOutputStream().write( (byte[])o );
				retval.flush();
			}
	...
	public String mkdir(MemoryFileItem item) throws IOException {
			...
			final String propName = item.getName();
			...
			parent.put( propName, ""+System.currentTimeMillis());
		...
		try {
			...
			for (String item : dir.keySet().toArray(new String[] {})) {
				if (item.endsWith("/.!")) {
					...
					final String pureName = dirName.substring(1,dirName.length()-2);
					list.add(pureName);					
				}else if (item.startsWith(folderUri +"/")) {
					final String dirName = item; 
					final String pureName = dirName.substring(folderUri.length()+1);
					list.add(pureName);
				}else if (item.startsWith("/")) {
					final String dirName = item; 
					final String pureName = dirName.substring(1);
					list.add(pureName);
				}else{
					list.add(item);
				}
	...
	private void remFromList(String name) {
		...
		if (name.startsWith("/")){
			parent.remove( name +"/.!" ); // DIR
		}
		else{
			parent.remove( pureName ); // FILE
		}
